...
jQuery(document)
		.ready(
				function($) {
					...
					var mediaPlayer = document.getElementById('videoPlayer'), playBtn = document
							.getElementById('play_btn'), stopBtn = document
							.getElementById('stop_btn'), volumeInc_btn = document
							.getElementById('volumeInc_btn'), replay_btn = document
							.getElementById('replay_btn');
					...
					function updateTime() {
						var currentTime = document.getElementById('currentTime'), time;

						if (mediaPlayer.duration < 3600) {
							time = new Date(mediaPlayer.currentTime * 1000)
									.toUTCString().replace(/.*(\d{2}:\d{2}).*/,"$1");
						} else {
							time = new Date(mediaPlayer.currentTime * 1000)
					 ...
					 * function muteVolume() {
					 * 
					 * if (mediaPlayer.muted) { //changeButtonType(this,
					 * 'Mute'); mediaPlayer.muted = false; } else {
					 * //changeButtonType(this, 'Unmute'); mediaPlayer.muted =
					 * true; }
					 *  }
					 */
					...
					function setProgressWidth() {
						...
						// GET THE WIDTH OF EACH ELEMENT IN THE MEDIA CONTROL
						// SECTION (INCLUDING THE 45PX OF SPACE INBETWEEN)
						// AND SUBSTRACT THEM FROM TOTAL WIDTH TO CALCULATE THE
						// SPACE LEFT AVAILABLE FOR THE PROGRESS BAR
						var widthAvailable = $('.media-controls').width()
								- $('#playControls').width()
								- $('#timeContainer').width()
								- $('#volumeControls').width() - 45 + "px";
						...
						slider
								.slider({
									...
									slide : function() {
										...
										// CHANGE BUTTON ICON ACCORDING TO
										// VOLUME LEVEL
